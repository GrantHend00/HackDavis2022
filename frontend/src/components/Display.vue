<template>
  <GoogleMap api-key="AIzaSyC2A2vs5L1PjZ9Q12G8fSfwxBUCTUvDyZU" style="width: 100%; height: 500px" :center="center" :zoom="15" 
    ...
    <InfoWindow v-else-if="riskLevel === 'Medium'" :options="{ position: center, content:  '<div style=color:orange;font-size:40px;font-weight:bold>Medium Risk</div>' }" />
...
export default defineComponent({
  ...
  setup(props) {
    ...
    if(riskNum.risk <= 0.5)
      riskLevel = "Low";
    else if(riskNum.risk > 0.5 && riskNum.risk <= 1)
      riskLevel = "Medium";
    else  
      riskLevel = "High"

    return { center, riskLevel };
  },
});
